...
void WriteLCD(unsigned char word, unsigned commandType, unsigned usDelay) {
	...
	// and enable the LCD for the correct command.
        ...
        LCD_D = (((word >> 4) << 12))|(LCD_D & 0x0FFF);
        EnableLCD(commandType, usDelay);
        LCD_D = (((word << 4) >> 4) << 12)|(LCD_D & 0x0FFF);
        EnableLCD(commandType, usDelay);
...
void LCDClear(void) {
	...
	WriteLCD(0x01, LCD_WRITE_CONTROL, 1);
}
...
void LCDInitialize(void) {
	...
	WriteLCD(0x2F, LCD_WRITE_CONTROL, 40);    
        // 0x28 = 0b0010'1111
	...
	WriteLCD(0x06, LCD_WRITE_CONTROL, 10);
        //0b0000'0110 = 0x06
	// Display On/Off Control
	// Turn Display (D) On, Cursor (C) Off, and Blink(B) Off
	WriteLCD(0x0C, LCD_WRITE_CONTROL, 10); // where D is on and C is off and B is off 0x0(1100)
        //0b0000'1100
} 
...
void LCDMoveCursor(unsigned char x, unsigned char y) {
	...
	// (x,y) coordinate.
        int i = 0;
        WriteLCD(0x02, LCD_WRITE_CONTROL, 1);
        if(x == 1)
            WriteLCD(0xC0, LCD_WRITE_CONTROL, 1);
        for(i = 0; i < y; i++)
            WriteLCD(0x14, LCD_WRITE_CONTROL, 1);
